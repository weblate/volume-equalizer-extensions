...
const isDefaultCrossoverFrequency = (frequency: number, defaultFrequency: number): boolean => {
  return Math.abs(frequency - defaultFrequency) <= defaultFrequency * 1e-6;
};

export const isEqualizerFilterEnabled = (filter: Partial<EqualizerFilter>): boolean => {
  ...
  if (filter.type === "highpass") {
    return Number.isFinite(frequency) && !isDefaultCrossoverFrequency(frequency, 20);
  }
  if (filter.type === "lowpass") {
    return Number.isFinite(frequency) && !isDefaultCrossoverFrequency(frequency, 20000);
  }
