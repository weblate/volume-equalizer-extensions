...
test("disconnects only its analyser while switching outputs and stopping", () => {
  ...
  sampler.start(
    contextA as unknown as AudioContext,
    outputA as unknown as AudioNode,
  );
  const firstAnalyser = contextA.analysers[0];
  sampler.start(
    contextB as unknown as AudioContext,
    outputB as unknown as AudioNode,
  );
...
test("reuses its analyser and sample buffers after a stop", () => {
  ...
  expect(context.analysers[0].frequencyBuffers[0]).toBe(
    context.analysers[0].frequencyBuffers[2],
  );
  expect(context.analysers[0].sampleBuffers[0]).toBe(
    context.analysers[0].sampleBuffers[2],
  );
